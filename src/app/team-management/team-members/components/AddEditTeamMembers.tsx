...
import {
  useCreateTeamMember,
  useUpdateTeamMember,
} from "@/api/hooks/team-member.hook";
import LoadingButtonCircle from "@/components/reusable/LoadingButtonCircle";
import { useForm, Controller } from "react-hook-form";
import Select from "react-select";
import Image from "next/image";
...
function AddEditTeamMembers({ isEdit, editData, onClose, onSuccess }: Props) {
  const { data: teamCategories } = useGetTeamCategories();
  const { data: roles } = useGetRoles();
  const { data: positions } = useGetPositions();

  const { mutateAsync: createTeamMember, isPending: createLoading } =
    useCreateTeamMember();
  const { mutateAsync: updateTeamMember, isPending: updateLoading } =
    useUpdateTeamMember();
  const isPending = createLoading || updateLoading;
  ...
  useEffect(() => {
    if (isEdit && editData) {
      setValue("memberName", editData.memberName ?? "");
      setValue("memberNameNp", editData.memberNameNp ?? "");
      setValue("email", editData.email ?? "");
      setValue("contact", editData.contact ?? "");

      // Only set a react-select option when we actually have an id,
      // otherwise react-select renders a hidden input with value={null}
      // which React warns about.
      if (editData.teamCategoryId) {
        setValue("teamCategory", {
          value: editData.teamCategoryId,
          label: editData.teamCategory,
        });
      ...
      if (editData.imageUrl) {
        setPreview(imageurlgenerator(editData.imageUrl));
      }
    }
  }, [isEdit, editData, setValue]);
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      formData.append("memberName", data.memberName);
      formData.append("memberNameNp", data.memberNameNp);
      if (data.email) {
        formData.append("email", data.email);
      }
      if (data.contact) {
      ...
      // Toast feedback is already shown by the mutation's onError handler.
      console.error("Failed to save team member:", error?.message || error);
    }
  };
    ...
    <AnimationWrapper>
      <div className="p-6 space-y-4 w-[50vw]">
        ...
        >
          {/* Member Name */}
          <div className="flex flex-col gap-2">
            <label className="form-label">Member Name (English)</label>
            <input
              {...register("memberName", { required: true })}
              placeholder="Enter member name"
              className="input-style"
          ...
          <div className="flex flex-col gap-2">
            <label className="form-label">Member Name (Nepali)</label>
            <Controller
              control={control}
              name="memberNameNp"
              render={({ field }) => (
